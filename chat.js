...
// Caps how many requests one visitor (by IP) can make in a short window.
// Note: Vercel may run several function instances, so this limit is applied
// per-instance rather than globally — it still meaningfully slows abuse.
// For a hard global limit, use Vercel KV / Upstash Redis (see README).
const RATE_LIMIT = 8;              // max requests...
const RATE_WINDOW_MS = 20 * 1000;  // ...per 20 seconds, per IP
...
export default async function handler(req, res) {
  ...
  // Identify the caller by IP (Vercel forwards it in this header).
  const ip = (req.headers['x-forwarded-for'] || '').split(',')[0].trim() || 'unknown';
  if (rateLimited(ip)) {
    return res.status(429).json({
      reply: "You're sending messages a little fast — give me a second and try again. If it's urgent, call (661) 400-6221."
    });
  }
  ...
  // Abuse guard: reject absurdly long single messages (a cost/DoS vector).
  const lastMsg = messages[messages.length - 1];
  if (lastMsg && typeof lastMsg.content === 'string' && lastMsg.content.length > 2000) {
    return res.status(400).json({
      reply: "That message is a bit long for me to process — could you shorten it? Or call us at (661) 400-6221."
    });
  }

  // Abuse guard: cap total conversation length (belt-and-suspenders with the trim).
  if (messages.length > 60) {
    return res.status(400).json({
      reply: "This chat's gotten quite long — let's pick it up by phone at (661) 400-6221 so we can help you faster."
    });
  }

  // Guard against runaway/abuse: cap history length sent to the model.
